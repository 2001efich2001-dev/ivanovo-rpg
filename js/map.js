    ...
    container.innerHTML = `
        <div style="position: relative; display: inline-block; width: 100%;">
            ...
            <svg class="map-overlay" viewBox="0 0 800 600" preserveAspectRatio="xMidYMid meet" style="position: absolute; top:0; left:0; width:100%; height:100%;">
                ...
                ${housingZones.map(z => `<circle cx="${z.cx}" cy="${z.cy}" r="${z.r}" data-location="${z.id}" data-name="${z.name}" data-type="${z.type}" class="housing-circle" fill="rgba(255,179,71,0.3)" stroke="rgba(255,179,71,0.8)" stroke-width="3" />`).join('')}
            </svg>
        </div>
    `;
